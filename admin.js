...
function savePreview() {
  try {
    ...
    message("保存容量がいっぱいです。写真を少なめにしてから、もう一度保存してください。");
  }
}
...
function setLabelValue(value) {
  ...
  const exists = [...labelSelect.options].some((option) => option.value === label);
  if (label && !exists) {
...
async function sendToSheet(silent = false, successText = "スプシとHPへ同期しました。反映まで数秒かかる場合があります。") {
  ...
  if (!url.startsWith("https://script.google.com/")) {
    if (!silent) message("保存しました。HPへ自動反映するには、自動反映の設定にApps Script URLを入力してください。");
    return false;
  }
  if (!token) {
...
async function syncAfterChange(localText) {
  ...
  if (!synced) message(`${localText} 自動反映の設定を確認してください。`);
}
